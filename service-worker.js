...
// ───────────────────────────────────────────────────────────────────────────
const CACHE_VERSION = 'cim-v3.9.04';   // fix297: assets/splash.jpg REGISTERED (900x1612 JPEG q82, 215 KB) for the boot splash screen landing in fix298. ASSET-ONLY FIX — STATIC_ASSETS gains exactly one entry, no HTML ships, nothing references the file yet. The bump is mandatory because STATIC_ASSETS changed and the manifest is only read at install: without it the old cache survives activation and the new file is never precached, so fix298 would 404 offline despite the entry being listed.
