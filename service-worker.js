...
// const CACHE_VERSION = 'cim-v3.9.27';   // fix322: TEAM LOGISTICS HUB, PART 1 — NAMED ROSTER (CIM-plan-fix322-team-logistics-hub-v1.md). Third content fix in the Directeur Sportif sub-series. Ships ONE mechanic: a fixed three-rider named roster (Elena Marchetti/Climber, Jamie Cole/Sprinter, Marcus Webb/Time-Trialist — PROVISIONAL flavor values), no hire/fire, no new roster-management economy. New DIRECTOR_ROSTER constant, a static table keyed by the SAME tt/hill/road terrain bucket directorStageRaceTypeId() already derives — zero new terrain taxonomy, zero new save-schema field, zero new load guard (it's a registry like DIRECTOR_STRATEGIES, not per-save state). Feeds the sim via terrain-matching (decision 2): whichever named rider matches the current stage's bucket applies a flat 1.15x specialization multiplier to ONE weighted component of simulateDirectorStageResult()'s existing playerBase calc — Climber/Time-Trialist boost the ftp component, Sprinter boosts the sprint component — BEFORE strat.weights combine them. getRiderStats() stays the sole underlying baseline (decision 3: additive depth on top of existing progression, not a parallel stat system) — attribute levels, equipment, and prestige all still matter exactly as before. Squad Form (fix321, state.directorForm) is completely untouched — formMult/formVarianceMult still apply to the combined score afterward, two independent layers on the same formula, checkDirectorStageResolve()'s debit/delta/regen logic byte-identical. UI: one new read-only roster strip in renderDirectorCalendar(), placed between the Squad Form meter and the Career Record card — three compact cards (icon/name/specialty/leads-label), no new interaction, no id attributes on any new element. One new INFO_TEXTS.directorRoster entry. Diff confined to: DIRECTOR_ROSTER constant addition, simulateDirectorStageResult()'s playerBase calc (raceTypeId lookup moved earlier, two new roster-boost reads, one duplicate raceTypeId declaration removed), renderDirectorCalendar()'s new rosterHtml block, and the one INFO_TEXTS entry. armDirectorStrategy(), checkDirectorStageResolve(), renderRaceTab()'s branching, renderLegacyFork(), and all Privateer-path code are byte-identical to fix321 — verified via diff-scope check (5 hunks total, all mapping to the four regions above).
// const CACHE_VERSION = 'cim-v3.9.28';   // fix323: TEAM FACILITIES (CIM-plan-fix323-team-facilities-v1.md). Fourth content fix in the Directeur Sportif sub-series, and the last piece needed to retire the fix320-era placeholder — Director stops reading getEquipmentBonusForRace() (the same flat personal-road-gear bonus a Privateer/personal-race player gets) and reads a new squad-facility system instead. THREE DESIGN DECISIONS LOCKED WITH MSK BEFORE CODE: (1) replace, not stack — a Director-only swap at one call site, Privateer/pre-fork scoring completely untouched; (2) acquisition is cash-purchase, tiered, same shape as BIKES/WHEELS (priced ladder gated by minPrestige), not a VP spend; (3) two facility types — Team Bus (climb-weighted) and Performance Lab (renamed from the working title "Wind Tunnel" to avoid colliding with the existing Skills-tab drill and Accessories timed-boost item of that name, both of which already feed wt_aero inside getEquipmentBonusForRace()). New TEAM_FACILITIES constant — a static table, same category as DIRECTOR_ROSTER/DIRECTOR_STRATEGIES, NOT per-save state — {bus:[tier1..5], lab:[tier1..5]}, each entry {id,tier,price,watts,minPrestige,desc}, PROVISIONAL price/watts numbers (flavour-and-balance pass, confirmed "use as-is" by MSK, tunable later without touching schema). New state.teamFacilities={bus:0,lab:0} stores a TIER INDEX per facility (0=not yet purchased, upgrade-track shape like state.attributes.<x>.level, NOT an owned-set like BIKES/WHEELS — a squad only ever has one bus and one lab) — own top-level field, own per-sub-key `??` load guard (tourProgress-lesson discipline: state.teamFacilities=state.teamFacilities||{}; .bus??=0; .lab??=0 — never a single top-level guard on the parent object), written whole in the save payload alongside directorForm. New getFacilityBonusForRace(raceTypeId) is a separate PURE function, parallel to and NEVER calling getEquipmentBonusForRace() — no climber passive, no D.accessoryWatts read, those stay personal-race concepts; reads TEAM_FACILITIES.bus[tier-1].watts and the lab equivalent (index 0 guarded to 0 watts), weighted through the EXISTING EQUIP_WEIGHTS[raceTypeId] split getEquipmentBonusForRace() already uses — Team Bus feeds the climb-weighted component (logistics/domestique support up mountains), Performance Lab feeds the aero-weighted component (data-driven aero optimisation). simulateDirectorStageResult() gains exactly ONE changed line — equipBonus=getEquipmentBonusForRace(raceTypeId) becomes equipBonus=getFacilityBonusForRace(raceTypeId) — function signature, AI-field loop, prize formula, Form/roster reads all byte-identical. New buyFacilityTier(facilityId) is THE single-funnel mutation point: resolves TEAM_FACILITIES[facilityId][state.teamFacilities[facilityId]] (the next tier), hard-lockout checks BEFORE the money debit in buyBike()'s exact order (legacyPath==='director', next tier exists i.e. not already maxed, minPrestige met, affordability) — single-tap, non-destructive spend, no confirm(), matches buyBike()/buyAccessory() precedent (two-tap confirm is reserved for destructive actions, this isn't one); safe no-op on every lockout, never throws, never debits. UI: buildEquipCard() gains a new directorLocked param, checked alongside the existing privateerLocked branch at the SAME rank (both reuse the EXISTING .prestige-locked visual class, label "Retired · Director path") — added to the same 5 call sites privateerLocked already touches (bikes catalogue, R&D prototypes, TT bikes, wheels, kit); Kit Colours picker deliberately NOT touched (cosmetic, unrelated to scoring, same reasoning Privateer's treatment already established). renderAccessoryShop() gains two new cards (Team Bus, Performance Lab) appended after the existing ACCESSORIES list — Director-only visibility (HIDDEN, not greyed, for Privateer/pre-fork players, since facilities are meaningless outside the Director path) — reusing buildEquipCard()'s existing owned/equipped/prestigeLocked branches rather than inventing new ones: a maxed facility renders as "owned+equipped" (no further action), an unmaxed one shows the next tier's real price through the normal affordable/locked branches. Single-stat variant per card (Bus supplies maxClimb only, Lab supplies maxAero only — no dual aero/climb bars on a squad asset that only ever feeds one weighted component). One new INFO_TEXTS.teamFacilities entry, wired to an ⓘ on the new section header. NO changes to getEquipmentBonusForRace() itself, armDirectorStrategy(), checkDirectorStageResolve(), renderRaceTab()'s branching, or renderLegacyFork() — Prestige Fork guardrail maintained. Diff confined to: TEAM_FACILITIES constant addition, state init/save/load for teamFacilities, getFacilityBonusForRace() beside getEquipmentBonusForRace(), simulateDirectorStageResult()'s one-line equipBonus swap, buyFacilityTier(), buildEquipCard()'s directorLocked param + its 5 call sites, renderAccessoryShop()'s new Team Facilities section, and the one INFO_TEXTS entry — verified via diff-scope check.
// const CACHE_VERSION = 'cim-v3.9.31';   // fix326: STAFF EFFICIENCY TESTS (CIM-plan-fix326-staff-efficiency-tests-v1.md). Seventh content fix in the Directeur Sportif sub-series. HTML ONLY -- no new or replaced assets, STATIC_ASSETS untouched. THREE DESIGN DECISIONS LOCKED WITH MSK BEFORE CODE: (1) trigger is tied to stage results, not VP-gated or player-initiated -- fires every 5 Director stages resolved, counted cumulatively across the whole career; (2) no new "staff" concept -- the EXISTING TEAM_FACILITIES bus/lab ladders (fix323) are tested directly; (3) Bus and Lab are tested INDEPENDENTLY, one roll each, weighted by that ladder's current tier (passChance = 0.50 + tier*0.07, PROVISIONAL like every other untuned figure in this sub-series), and a pass/fail sets a ROLLING +-15% modifier that persists until that ladder's NEXT test replaces it (not additive across cycles, not decaying after one stage). New directorCareer.stagesResolved is a cumulative, permanent counter -- own sub-key inside the EXISTING directorCareer per-sub-key `??` guard, same tourProgress-lesson discipline as toursDirected/gcPodiumsAsDirector already there; a save from fix325 or earlier heals to 0. New state.staffTestModifier={bus:0,lab:0} is its own top-level sibling of teamFacilities (NOT nested inside it), own per-sub-key `??` load guard; PERMANENT like teamFacilities itself -- untouched by checkCalendarSeasonReset()/handlePrestige(), same as teamFacilities already is. New checkStaffEfficiencyTest() is THE single write site for both new fields: increments stagesResolved every call, and every 5th call rolls independently per ladder (a ladder still at tier 0/unpurchased is skipped silently -- nothing to test). Called from checkDirectorStageResolve() immediately after state.directorStageResolvedToday is set true, same tick as the existing Squad Form debit. Returns a summary string rather than calling showToast() itself -- showToast() has no queue, and checkDirectorStageResolve() already fires its own end-of-function toast, so two showToast() calls in the same tick would clobber each other; the summary is folded into that single existing toast instead (` . ` separator when a test fired that tick). getFacilityBonusForRace() (fix323) gains two multiplier lines applying the rolling modifier on top of the existing tier-wattage read -- the only change to that function, the bus/lab tier lookup itself untouched. Team Facilities screen (renderAccessoryShop()) gains a small read-only "Efficiency: +-N%" line appended after each facility's own card when that ladder's modifier is nonzero -- NOT inside buildEquipCard() itself (shared with gear/bike cards elsewhere, untouched), no tap target, no state write, render-only. INFO_TEXTS.teamFacilities gains one paragraph explaining the test cadence and effect. Diff confined to: directorCareer schema/init/guard (stagesResolved), staffTestModifier schema/init/guard/save-payload (own top-level field), the new checkStaffEfficiencyTest() function, checkDirectorStageResolve()'s one new call line + toast-string interpolation, getFacilityBonusForRace()'s two multiplier lines, renderAccessoryShop()'s two new read-only modifier lines, and the one INFO_TEXTS.teamFacilities paragraph -- verified via diff-scope check against the fix325 baseline.
// const CACHE_VERSION = 'cim-v3.9.32';   // fix327: MEDICAL WING (CIM-plan-fix327-medical-wing-v1.md). Eighth and final content fix in the Directeur Sportif sub-series' sequenced roadmap. HTML ONLY -- no new or replaced assets, STATIC_ASSETS untouched. DESIGN-QUESTION PASS HELD WITH MSK BEFORE CODE, SIX DECISIONS LOCKED: (1) scope is SQUAD-WIDE, one state, mirroring directorForm -- explicitly NOT per-rider, DIRECTOR_ROSTER stays a static registry untouched by this fix; (2) trigger is a THRESHOLD read off directorForm (MEDICAL_FORM_THRESHOLD=30, provisional), not a random roll or stage-count interval like fix326 -- a setback arms only when directorForm drops below the threshold AND no setback is already active, so one setback at a time, no re-trigger/extension while one is running; (3) effect is a flat watts penalty while active (MEDICAL_SETBACK_PENALTY=-0.12, provisional), applied as a multiplier on the TOTAL squad score in simulateDirectorStageResult() -- not folded into playerBase/equipBonus individually, since the setback is a squad-wide condition, not facility- or roster-specific; (4) recovery is a PASSIVE TIMER (MEDICAL_RECOVERY_STAGES=3, provisional), decremented once per stage resolved, same call-site tick as fix326's checkStaffEfficiencyTest(); (5) EXPEDITED recovery is a player choice between VP or cash, both deliberately PUNISHING per MSK's explicit call and LOCKED (not provisional): 25 flat VP, or 35% of the player's CURRENT state.money computed LIVE at spend time (never stored) so it stays painful at any point in a run rather than trivial late-game; (6) directorCareer gains a new lifetime counter, setbacksSuffered, incremented once per setback triggered -- matches fix320's "richer career stats are a Medical Wing concern" comment, same permanent/append-only discipline as stagesResolved. New state.medicalSetback={active:false,stagesRemaining:0} is its own top-level field, own per-sub-key `??` load guard (tourProgress-lesson discipline) -- UNLIKE staffTestModifier/teamFacilities this is NOT permanent: it resets to {active:false,stagesRemaining:0} at BOTH reset sites where directorForm itself resets to 70 (checkCalendarSeasonReset() AND handlePrestige() -- confirmed during implementation that directorForm resets at BOTH boundaries, not prestige-only as the plan doc's initial pass assumed; medicalSetback was corrected to match the real code rather than the doc). New checkMedicalWing() is one of two write sites for medicalSetback (the stage-tick-initiated one): mirrors checkStaffEfficiencyTest()'s exact pattern, called from checkDirectorStageResolve() immediately after the existing Form debit so the threshold check reads THAT stage's post-debit Form; returns a summary string rather than calling showToast() itself, folded into the same single end-of-function toast staffTestSummary already feeds. New expediteMedicalRecovery(currency) is the OTHER write site (player-initiated): VP spend goes through the existing spendVP() single funnel, cash spend is a direct state.money debit mirroring every other buy*() function's discipline (no shared spendMoney() funnel exists in this codebase). New requestExpediteMedicalRecovery(currency) is a two-tap arm/confirm wrapper, no confirm(), mirroring requestQuitFeedZone()/requestQuitSlipstream() exactly, adapted for two currency buttons sharing one armed slot (arming one disarms the other) -- safe because renderDirectorCalendar() is event-driven (arm/resolve/tab-switch), never ticked, confirmed by inspecting its existing countdown-patch code before relying on DOM-held arm state surviving between taps. renderDirectorCalendar() gains a new read+action panel, placed directly below the existing Squad Form meter and before the roster strip -- narratively adjacent since a setback is a direct consequence of Form; renders NOTHING when no setback is active (no permanent "all clear" clutter, same precedent as fix326's "no toast for an untested ladder"), render-only otherwise, no state writes. New INFO_TEXTS.medicalWing entry for the panel's (i) button. This is the LAST of the four items in the Directeur Sportif sub-series roadmap doc's sequenced list (Team Logistics Hub, Race Radio Scramble, Staff Efficiency Tests, Medical Wing) -- soft-launch backlog remains separately gated on D1/D7 retention data. Diff confined to: five new top-level constants beside TEAM_FACILITIES, directorCareer schema/init/guard (setbacksSuffered) + its two reset-site exemptions, medicalSetback schema/init/guard/save-payload/BOTH reset sites (own top-level field), the new checkMedicalWing()/expediteMedicalRecovery()/requestExpediteMedicalRecovery() functions, checkDirectorStageResolve()'s one new call line + toast-string interpolation, simulateDirectorStageResult()'s one new multiplier line, renderDirectorCalendar()'s new medicalHtml block + its slot in the assembled markup, and the one new INFO_TEXTS.medicalWing entry -- verified via diff-scope check against the fix326 baseline.
const CACHE_VERSION = 'cim-v3.9.33';   // fix328: DEV MODE -- SNAPSHOT SANDBOX (CIM-plan-devmode-v1.md). Temporary testing-only feature, explicitly NOT part of the Directeur Sportif sub-series (which closed at fix327) or the soft-launch backlog -- built so MSK can reach and exercise post-triple-GT states (the Prestige Fork choice, and specifically the Director path fix320-327 live behind) without grinding a season for real. HTML ONLY -- no new or replaced assets, STATIC_ASSETS untouched. Every new symbol is tagged DEV_MODE_ENABLED across the HTML file (constant beside SAVE_KEY, core functions block after applySettings(), renderSettings() panel addition, boot call, password modal markup + CSS near #info-overlay) for a one-pass tag-based removal before soft-launch -- scattered by necessity (constants near the top, functions near their conceptual siblings, markup in the body) the same way every other fix's own "// fixNNN" comments are scattered, not a literal contiguous byte range. DESIGN-QUESTION PASS HELD WITH MSK BEFORE CODE: (1) trigger is a long-press (~1.4s) on the version readout in Settings, delegated via one pointerdown/pointerup listener installed once at BOOT rather than bound to the version row's own node, since renderSettings() rebuilds that node wholesale on every call; (2) gate is a custom password modal (no prompt()), password is a plain hardcoded string ('DevDevDev') -- a discreet convenience gate, not real security, explicitly temporary and stripped with everything else before soft-launch; (3) snapshot scope is the ENTIRE state object, captured GENERICALLY via a Set/Map-aware JSON replacer/reviver pair (devSerializeState/devDeserializeState) rather than a curated field list -- deliberately avoids the per-field-whitelist drift risk saveGame()'s own payload has, since this needs no maintenance as state grows; (4) storage is ONE dedicated localStorage key (DEV_SNAPSHOT_KEY, separate from SAVE_KEY) whose mere EXISTENCE is the active flag -- saveGame()/loadGame() are NEVER edited, so an active dev session's live progress persists across app close/reopen for free, through the ordinary save, exactly as a real session would; (5) the "skip to the fork choice" dev action (devGrantTripleGT()) grants the REAL trigger condition (adds all three PERFECT_SEASON_TOURS ids to state.seasonAchievements.gtWon) and calls the EXISTING checkLegacyForkEligibility() funnel unchanged -- the real fork UI fires, no synthetic bypass, per MSK's explicit "play as intended" call; single-tap, since the whole dev session is already disposable by construction; no-ops once state.legacyPath is already set, mirroring that funnel's own early-out; (6) exit (devExitTap() -> devRestoreAndExit()) is two-tap arm/confirm (same SET_UPD_ARM_MS-style pattern as settingsUpdateTap()), since it discards real dev-session progress -- restores the snapshot by mutating the live `state` object IN PLACE (state is `const`, never reassigned: delete every own key, then Object.assign from the deserialized snapshot), calls the EXISTING saveGame() to persist the restored state through its normal whitelist (self-correcting against the real schema rather than trusting the generic clone's shape directly), clears DEV_SNAPSHOT_KEY, then location.reload() -- deliberate, not a shortcut: boot's own loadGame()->applySettings()->hub-render sequence is the only place already proven to rebuild every cache correctly from a fresh state, matching the existing settingsUpdateTap() precedent of reloading as its own second-tap action. Diff confined to: one new constant (DEV_SNAPSHOT_KEY) beside SAVE_KEY, one new tagged functions block after applySettings() (devSerializeState/devDeserializeState/devOpenPwModal/devClosePwModal/devPwSubmit/devGrantTripleGT/devExitTap/devRestoreAndExit/devInstallLongPress + their module-level state), renderSettings()'s one new conditional panel append, one new boot call to devInstallLongPress(), one new password-modal overlay + its CSS -- verified via diff-scope check against the fix327 baseline: six hunks, ALL pure additions (no existing line modified or deleted). checkLegacyForkEligibility() and saveGame() are called, never edited. No new saveGame()/loadGame() payload field -- dev-active status is derived from DEV_SNAPSHOT_KEY's presence, never trusted from memory across a reload.
// const CACHE_VERSION = 'cim-v3.9.30';   // fix325: RACE RADIO SCRAMBLE, STAGE 2 + CHAIN REWARD (CIM-plan-fix325-race-radio-scramble-stage2-v1.md). Sixth content fix in the Directeur Sportif sub-series -- the SECOND half of the two-part chain fix324 split off (Stage 1, "Radio Scramble", plus the gate flag, shipped there). HTML ONLY -- no new or replaced assets, STATIC_ASSETS untouched. Every locked decision from fix324's plan (mechanic shape, two chained stages, no-partial-credit, upfront VP for the whole chain, single-use directorRadioBoost flag) carries over unchanged -- this fix finishes that approved plan rather than reopening it. Reskin approach is IDENTICAL in technique to fix324's Stage 1: the bt-overlay DOM/CSS and every BT_* constant/btStart()/btEvaluateThrow()/btEnd() are reused WHOLESALE and stay byte-identical in physics/scoring math for the ORIGINAL Workshop-tab Bidon Toss -- a new btTheme flag ('default'|'director_radio') plus a small BT_THEMES copy registry and btApplyTheme() swap ONLY text strings at open time; icon stays 'drink' (a bidon reads fine literally, no new icon needed). Seven bt-overlay DOM nodes (head label, start title/copy/button, both end-panel buttons, two legend spans, brief title/copy) gained ids so btApplyTheme()/btEnd() can retarget them per theme; the 'default' theme's copy is the original fix245/fix246 text verbatim, so the Workshop-tab entry (startRiderSkillGame('bt') -> openBidonToss() with no arg) is visually and behaviourally unchanged. New armDirectorSupplyRiders(tourId) is Stage 2's entry funnel: Director-only, requires the SAME live ds.resolveAt window armDirectorRadioScramble() checks AND ds.radioStage1Cleared === true, blocked once state.directorRadioBoost[tourId] is already true (chain already banked this window) -- charges NO further VP, since the one chain cost was already spent upfront at Stage 1 opt-in. btStart()'s existing rsCanStart('bt') lockout (the ORIGINAL Bidon Toss's own bt_hype buff-lock) is skipped for the director_radio theme, mirroring fzStart()'s fix324 change. btEnd() branches fully on theme for its post-game write: the director_radio path touches NOTHING in rsStore() (no bestScores.bt, no lastPlayed.bt, no markTaskProgress('drill_bt'), no bt_hype buff, no rsShowSuccessPop('bt')) -- reuses Bidon Toss's own existing pass bar (BT_HYPE_THRESHOLD, 6/10) rather than inventing a new number, same principle as Stage 1 reusing FZ_THRESHOLD outright; a win sets state.directorRadioBoost[tourId]=true and repaints renderDirectorCalendar(), a loss changes nothing further -- ds.radioStage1Cleared is deliberately NOT revoked on a Stage 2 loss, so the player may retry Stage 2 for free within the same still-live cooldown window. The end panel's retry button is hidden for the director theme, same reasoning as fix324's Stage 1 (retries route back through the calendar card); closeBidonToss() now routes to showTab('race')+renderDirectorCalendar() instead of openRiderSkillsHub() when the theme is director_radio, and resets btTheme/_directorRadioTourId back to defaults on the way out (the SAME transient _directorRadioTourId Stage 1 already owns -- both stages of one chain track the same live tour). New state.directorRadioBoost is its own top-level sibling of directorSeason (NOT nested inside a ds entry, per the plan's explicit tourProgress-lesson call-out matching directorForm's own precedent) -- one boolean sub-key per the three GRAND_TOUR_STAGES tourIds, own per-tourId `??` load guard, written whole in the save payload, reset to all-false at the same two season-boundary sites directorSeason/directorForm already reset at (checkCalendarSeasonReset()/handlePrestige()). checkDirectorStageResolve() reads and clears the resolving tour's directorRadioBoost flag in the SAME single-write-site pass where ds.radioStage1Cleared is defensively cleared, then threads a new boosted param into simulateDirectorStageResult(), which computes formMult/formVarianceMult off an effective-Form value (Math.min(100,directorForm+27) instead of the raw meter) when boosted -- the SAME two lines fix321 already owns, just fed a different input -- rather than a parallel bonus term; +27 effective-Form points is a flat PROVISIONAL number (about +0.08 to formMult at the 70 baseline), tunable later without touching the caller. state.directorForm's real baseline is never written by this. UI: renderDirectorCalendar()'s existing radio-scramble slot becomes a three-way branch (Stage 1 entry button -> Stage 2 entry button once cleared -> a banked badge once the boost lands), replacing fix324's two-way placeholder. Diff confined to: BT_THEMES/btTheme/btApplyTheme(), openBidonToss()'s theme param, btStart()'s one-line lockout guard, btEnd()'s full theme branch, closeBidonToss()'s theme branch, the new armDirectorSupplyRiders(), checkDirectorStageResolve()'s boost read/clear + the boosted param threaded into simulateDirectorStageResult(), simulateDirectorStageResult()'s effectiveForm calc, the seven #bt-overlay id additions, state.directorRadioBoost (init/save/load/two season-reset sites), and renderDirectorCalendar()'s three-way action branch -- verified via diff-scope check against the fix324 baseline.
// const CACHE_VERSION = 'cim-v3.9.29';   // fix324: RACE RADIO SCRAMBLE, STAGE 1 (CIM-plan-fix324-race-radio-scramble-v1.md). Fifth content fix in the Directeur Sportif sub-series -- the FIRST of a two-part chain (Stage 2, "Supply the Riders", a Bidon Toss reskin, plus the chain reward, lands in fix325). HTML ONLY -- no new or replaced assets, STATIC_ASSETS untouched. TWO DESIGN DECISIONS LOCKED WITH MSK BEFORE CODE: (1) VP cost 2 (down from the plan's provisional 8); (2) shipped as two fixes, not one -- this fix carries Stage 1 ("Radio Scramble") and the gate flag only, no reward yet. Reskin approach per the plan: the fz-overlay DOM/CSS and every FZ_* constant/fzStart()/fzSpawnItem()/fzEnd() are reused WHOLESALE and stay byte-identical in spawn/physics/scoring math -- a new fzTheme flag ('default'|'director_radio') plus a small FZ_THEMES copy/icon registry and fzApplyTheme() swap ONLY text strings and icn() calls at open time. Ten fz-overlay DOM nodes (head label, time/score label+suffix, start title/copy/button, three legend spans, brief title/copy, both end-panel buttons) gained ids so fzApplyTheme()/fzEnd() can retarget them per theme; the 'default' theme's copy is the original fix151 text verbatim, so the Workshop-tab entry (startRiderSkillGame('fz') -> openFeedZone() with no arg) is visually and behaviourally unchanged -- verified no new duplicate ids against the fix323 baseline. New armDirectorRadioScramble(tourId) is THE entry funnel: Director-only, requires a live ds.resolveAt window (same window armDirectorStrategy() just opened) AND !ds.radioStage1Cleared, spends DIRECTOR_RADIO_VP_COST=2 via the existing spendVP() funnel UPFRONT -- win or lose, the VP is not refunded (locked decision) -- then opens the themed overlay via openFeedZone('director_radio'). fzStart()'s existing rsCanStart('fz') lockout (the ORIGINAL Feed Zone's own fz_regen buff-lock) is now skipped for the director_radio theme specifically, so the two entry paths can never block one another -- they are deliberately independent mechanics sharing only the timing/spawn engine. fzEnd() branches fully on theme for its post-game write: the director_radio path touches NOTHING in rsStore() (no bestScores.fz, no lastPlayed.fz, no markTaskProgress('drill_fz'), no grantBuff('fz_regen',...), no rsShowSuccessPop('fz')) -- a win instead sets ds.radioStage1Cleared=true on the CURRENT tour's ds (tracked via the transient, never-saved _directorRadioTourId) and repaints renderDirectorCalendar(); this keeps the reskin's reward chain fully separate from the original Feed Zone's Rider-Skills bookkeeping, no Path A/Path B-style bleed between the two. The end panel's "Ride it again" button is hidden for the director theme (VP is charged per attempt, so a free in-overlay replay would undercut that) -- the player closes back out to the calendar card to pay and try again; closeFeedZone() now routes to showTab('race')+renderDirectorCalendar() instead of openRiderSkillsHub() when the theme is director_radio, and resets fzTheme/_directorRadioTourId back to defaults on the way out so no state can leak into a future normal Workshop open. New ds.radioStage1Cleared: own per-tour sub-key inside the EXISTING directorSeason per-sub-key `??` load guard (tourProgress-lesson discipline -- a save from fix323 or earlier carries no radioStage1Cleared at all and heals to false); reset to false at the START of every new stage window in armDirectorStrategy() (a clear from a previous window must never carry forward) and defensively cleared again in checkDirectorStageResolve() when the window resolves. UI: renderDirectorCalendar()'s existing armed-window branch gains one new line beneath the strategy countdown -- an entry button ("Radio Scramble . 2 VP") while unclaimed, a cleared badge once won; nothing renders outside the live resolve window, matching the plan's "optional, skippable, no penalty" framing. NOT YET WIRED TO ANYTHING: ds.radioStage1Cleared is written and displayed but has no consumer yet -- Stage 2 (fix325) is what reads and eventually clears it via the real chain reward (state.directorRadioBoost). Diff confined to: DIRECTOR_RADIO_VP_COST constant, the #fz-overlay id additions, fzTheme/FZ_THEMES/fzApplyTheme()+openFeedZone()'s theme param, fzStart()'s one-line lockout guard, fzSpawnItem()'s icon lookup, fzEnd()'s full theme branch, closeFeedZone()'s theme branch, armDirectorStrategy()'s one-line gate reset, the new armDirectorRadioScramble(), checkDirectorStageResolve()'s one-line gate reset, the directorSeason load guard's new sub-key, and renderDirectorCalendar()'s new prompt/badge line -- verified via diff-scope check (13 hunks against the fix323 baseline, all mapping to the regions above).
// fix321: SQUAD FORM (CIM-plan-fix321-squad-form-v1.md). Second content fix in the Directeur Sportif sub-series. Ships ONE mechanic: state.directorForm, a single squad-wide meter (0-100, baseline 70, hard floor 20/ceiling 100, its own top-level field with its own load guard — never nested under directorSeason/directorCareer, tourProgress-lesson discipline). Moved by two forces on the SAME single write site, checkDirectorStageResolve(): a per-strategy fatigue debit (new DIRECTOR_STRATEGIES[*].formCost field, additive to the existing weights/varianceMult/id/name/icon/desc — protect_gc:4, sprint_train:6, send_breakaway:10) and a result delta sized by placement (top-3 +10, top-10%-of-field +4, bottom-quartile -6, else 0), net change clamped [20,100] at the write site same as state.energy. Rest-day regen hangs off the EXISTING checkDailyReset() day-boundary block (no new tick): if no Director stage resolved that calendar day (tracked via new state.directorStageResolvedToday, day-boundary bookkeeping only, its own guard), Form drifts 3 points toward the 70 baseline from either side. Read (not written) in simulateDirectorStageResult() as formMult (0.85-1.15, multiplies playerBase alongside the existing vo2Mult) and formVarianceMult (1.3x-0.7x, multiplies the existing variance calc alongside strat.varianceMult) — no signature change, no AI-field-loop change, no prize-formula change. Resets to 70 at the same two season-boundary sites directorSeason already clears at (checkCalendarSeasonReset()/handlePrestige()) — directorCareer's permanent counters untouched by either. No lockout at the Form floor (MSK-confirmed: "let the numbers speak") — a low-Form squad just races worse and swingier, strategies always stay armable. UI: one Form meter (battery icon, bar+number, reuses the existing .energy-bar-outer/-inner CSS — no new classes) in renderDirectorCalendar(), shown ONCE above the per-tour cards since Form is squad-wide, never duplicated per card. One new INFO_TEXTS.squadForm entry. Diff confined to: DIRECTOR_STRATEGIES formCost additions, state init/save/load for directorForm+directorStageResolvedToday, the two season-reset call sites, checkDirectorStageResolve()'s debit/delta calc, checkDailyReset()'s existing day-boundary block (regen), simulateDirectorStageResult()'s two new multiplier reads, renderDirectorCalendar()'s meter markup, and the one INFO_TEXTS entry. Privateer-path code, renderLegacyFork(), renderRaceTab()'s branching, and the road GT calendar are byte-identical to fix320 — verified via diff-scope check against the fix320 baseline (11 hunks, all mapping to the regions above). // fix320: DIRECTEUR SPORTIF PATH — RACE DAY AUTO-RESOLVE (CIM-plan-fix320-director-v1.md). First content fix on Path B. Ships ONE mechanic: Race Day Auto-Resolve with three Strategy Sliders (Protect the GC / Send the Breakaway / Sprint Train — DIRECTOR_STRATEGIES weight+varianceMult table). Player stays hybrid post-fork: personal rider card preserved, but startRace()/simulateRaceResult() are never invoked again once legacyPath==='director' — zero new branches in either. New state: directorSeason (season-scoped, cleared at the same two sites privateerSeason clears — checkCalendarSeasonReset()/handlePrestige() — one entry per real GRAND_TOUR_STAGES tour; tour_de_suisse is a one-day CALENDAR_EVENTS entry with no stage list and is deliberately out of scope), directorCareer (permanent, append-only — toursDirected/gcPodiumsAsDirector). Squad strength is getRiderStats() read as a proxy (decision #6) — no new roster structure; that's the future Team Logistics Hub fix. Sim (simulateDirectorStageResult()) mirrors simulatePrivateerRaceResult()'s exact shape — same prestigeMult/legacyPrizeMult/diffMod prize formula, no new RNG surface beyond the strategy weight/variance table; raceTypeId is derived from the stage's own terrain string solely to pick which getEquipmentBonusForRace() table to read, never to alter scoring. Cadence: hangs off the EXISTING checkDailyReset() 60s interval at the exact call site checkCalendarSeasonReset() already occupies — no new setInterval. Strategy cost is VP via the existing spendVP() funnel. UI: renderRaceTab() branches a third way into new renderDirectorCalendar() in #director-calendar-wrap, sibling to fix319's #privateer-ladder-wrap, same .privateer-active-shaped .director-active CSS toggle. Road gear cards are UNCHANGED for Director players — no locked/greyed treatment, since Director keeps reading the same equipment bonuses. Diff confined to: state init/save/load, the two season-reset call sites, DIRECTOR_STRATEGIES/DIRECTOR_STAGE_VP_COST/DIRECTOR_RESOLVE_MS, directorStageRaceTypeId()/simulateDirectorStageResult()/armDirectorStrategy()/checkDirectorStageResolve() + its checkDailyReset() hookup, directorTickCountdowns() + its gameTick() hookup, renderRaceTab()'s third branch, renderDirectorCalendar(), the #director-calendar-wrap markup+CSS, renderLegacyFork()'s Director copy, and one INFO_TEXTS entry. completeRace(), the interactive stage/decision engine, the road GT calendar, and every Privateer-path function/field are untouched. (CIM-plan-fix319-privateer-v1.md). First content fix on the fix318 Prestige Fork schema. Ships the full four-rung ladder (Qualifier top-5 → Nationals top-3 → Worlds top-1 → Lifetime Grand Prix top-1, MSK-confirmed tightened thresholds) plus a separate gravel gear inventory (GRAVEL_BIKES/GRAVEL_WHEELS/GRAVEL_TYRES, single gravelWatts stat, no prestige gating). New state: privateerSeason (season-scoped, cleared at both checkCalendarSeasonReset() and handlePrestige()), privateerCareer (permanent, append-only — Lifetime GP win count is a split: the season attempt resets, the career counter does not), gravelGear (owned/equipped inventory, starter items free by default). Sim reuses simulateCalendarEventResult()'s exact shape through new simulatePrivateerRaceResult(), fed ONE new reweighted terrain profile (PRIVATEER_TERRAIN, gravel/endurance-leaning — the only new sim input) rather than a new RNG surface. Deliberate scope line: Privateer races resolve on a single tap via new enterPrivateerRace(), NOT through the interactive stage/decision overlay — completeRace() gains zero new branches and is byte-identical to fix318, protecting the road GT calendar from any regression risk. UI: renderRaceTab() branches on legacyPath into new renderPrivateerLadder(), a plain (non-painted) panel — sited outside the Gear tab's five-medallion hub deliberately, since a sixth medallion needs new painted-art hotspot geometry this session has no art for. Road gear cards (BIKES/WHEELS/KITS) gain one new boolean, privateerLocked, reusing the EXISTING .prestige-locked visual class — no new CSS class for that. One small new CSS block for the ladder panel's show/hide toggle. NO new or replaced assets — STATIC_ASSETS untouched, same as fix318. Diff confined to: state init/save/load, the two season-reset call sites, GRAVEL_* registries + buy/equip funnels, PRIVATEER_TERRAIN/PRIVATEER_LADDER, simulatePrivateerRaceResult()/checkPrivateerLadderAdvance()/enterPrivateerRace(), renderPrivateerLadder()/buildGravelEquipCardHtml()/buildGravelGearSectionHtml(), buildEquipCard()'s privateerLocked branch + its 5 call sites, the #privateer-ladder-wrap markup+CSS, and one INFO_TEXTS entry.
// const CACHE_VERSION = 'cim-v3.9.22';   // fix317: CONSUMABLE MARGINAL GAINS — MINI-GAME FUNNEL, HTML ONLY (CIM-plan-rewards-overhaul-v1 §3, plan now fully shipped). Three blocking design questions resolved by MSK this session before any code: (1) tap-to-use picker, not passive-on-owned — a consumable is spent at the tap; (2) uncapped stacking; (3) moderate effect strength across all four, PROVISIONAL like every other untuned figure in this plan. A FOURTH question surfaced reading the code, not in the plan: §3's literal UI-placement wording ("Workshop tab/gear-section-workshop") doesn't match where the four usableIn hook sites live — that div renders the unrelated Workshop bench games (wsAwardMG funnel), not the Rider Skills games (Feed Zone/Slipstream/Wind Tunnel/Bidon Toss) the consumables actually work in. Flagged and resolved: the tap-to-use picker lives on each game's own start panel (Skills tab, where a run is actually armed); a separate read-only inventory display goes in the Gear tab's Extras section (gear-section-accessories) per MSK's explicit call — neither the literal Workshop wording nor the Skills hub. New useConsumable(id, tag) is THE sole decrement site for state.consumables, mirroring buildRdBike()'s discipline for rdMaterials (fix316); tag reuses CONSUMABLES[].usableIn verbatim rather than a second short-tag registry that could drift from it. One armed slot per game (fzArmedConsumable/slArmedConsumable/wtArmedConsumable/btArmedConsumable), transient lets never saved, same pattern as fzScore/wtScore/slScore. cancelConsumable() is the only way back and refunds THROUGH grantConsumable() (fix315's existing earn funnel) rather than writing state.consumables directly, so spend and earn each keep exactly one writer; a dangling arm (tapped, run never started) is cleared with no refund in all four close*() functions. Four effects, each applied at the narrowest point that avoids the hot per-frame loops: focus_gel slows item fall in feedzone (FZ_GEL_SPEED_MULT=0.75, read fresh per spawned item); chain_lube and draft_tip apply a x1.15 score multiplier once at finalisation in wtEnd()/slEnd() (windtunnel's clamped to 100 since wtScore is a %, slipstream's uncapped); extra_bidon adds BT_BIDON_EXTRA_THROWS=2 to a new per-run btThrowBudget set in btStart(), replacing the fixed BT_THROWS at the one gameplay check-site and the live throws-remaining display — btAward()'s pct denominator and bestScores.bt DELIBERATELY stay on the fixed BT_THROWS=10 scale (Extra Bidon's reward effect is raising the ceiling on that ratio, clamped at 1.0, not a second bonus), commented at both sites; btEnd()'s "X of Y" display text uses btThrowBudget so it always matches what actually happened on the board. New renderConsumablePicker(tag) serves all four games from one function (three states — armed/owned/empty — identical shape, only the container id and CONSUMABLES lookup differ); reuses .rd-chip/.rd-chip.mat-selected/.rd-chip.locked (fix316) rather than inventing new CSS — NO new CSS rules added. Four new empty picker-container divs, one per start panel. NO NEW STATE FIELDS — state.consumables has existed schema-only since fix313 and earning since fix315; this is its first spend/read site, load guard unchanged. No save migration, no new or replaced assets, STATIC_ASSETS untouched. Diff confined to: four HTML picker-container divs, the new shared funnel block sited before Feed Zone, four new per-game effect constants, four openXXX() render-call additions, the four effect-application sites, and the four close*() dangling-arm clears — nothing else touched.
// const CACHE_VERSION = 'cim-v3.9.21';   // fix316: R&D MATERIALS SPEND HOOK, HTML ONLY (CIM-plan-rewards-overhaul-v1 §2, final piece). MSK resolved both blocking design calls this session: (1) ceiling-only, no new RNG surface — buildRdBike()/computeRdStats() stay fully deterministic; (2) BOTH tiers exist — common materials are stockpile/spend, rare materials are own-to-unlock permanent passives. RD_MATERIALS gains stat ('aero'|'climb') and bonus (flat watts) fields, PROVISIONAL like every other untuned figure in this plan. New computeRdMaterialBonus(selectedCommonIds) is a SEPARATE additive function beside the untouched, still-pure computeRdStats() — never folds materials into that function, so the base chip-picker model is byte-identical to fix308. Common materials only contribute if BOTH selected (new module-level `rdMaterialUse`, mirrors rdBuildSelection's non-persisted pattern) AND still owned; rare materials contribute automatically the moment state.rdMaterials[id]>0, with no selection UI at all, and are NEVER decremented by any funnel — owning 2+ of one rare id still counts once, this is not a stockpile. buildRdBike() gains a third param (materialUse) and is now the SOLE spend site for common materials — the decrement joins the same atomic write as the new rdBikes entry + rdBuildSeason stamp + saveGame(), so a spend and a build can never come apart; resolves usedCommon/activeRare BEFORE spending (a stale toggle on something no longer owned silently drops instead of going negative), folds the bonus into the stored aeroWatts/climbWatts, and records materialsUsed/activeRare + an "Enhanced with..." desc suffix on the entry for transparency. rdBuildConfirm() passes rdMaterialUse through and clears it after a committed build. renderRdBuildCard(): new Materials row (common, tappable chips, disabled when count is 0, rdToggleMaterial() arms/disarms) and Rare Materials row (read-only span, "active perk" when owned else "own 1 to unlock") sited between the axis rows and the stat-bar preview; preview stats now include computeRdMaterialBonus() so the bars are truthful to what a confirmed build will actually store. A compact read-only inventory tally (matTallyHtml) renders in BOTH the open and season-locked card states, so the stash stays visible year-round. Four new CSS rules beside .rd-chip.active: .locked (muted, covers un-owned common AND rare), .mat-selected (amber, "armed to spend"), .rare-active (green, "owned/unlocked"), .rd-chip-tally (non-interactive inventory line). NO NEW STATE FIELDS — state.rdMaterials already existed schema-only since fix313; this is its first spend/read site. No migration, no new or replaced assets, STATIC_ASSETS untouched. Diff confined to: RD_MATERIALS's two new fields, the new computeRdMaterialBonus() beside computeRdStats(), rdMaterialUse + rdToggleMaterial() beside rdBuildSelection, buildRdBike()'s spend logic, rdBuildConfirm()'s pass-through+reset, renderRdBuildCard()'s materials markup, and the four new .rd-chip CSS rules — nothing else touched.
// const CACHE_VERSION = 'cim-v3.9.20';   // fix315: TIERED WEEKLY REWARD TABLE, HTML ONLY (CIM-plan-rewards-overhaul-v1 §4). The Daily Challenge now rolls a material/consumable drop on top of its existing VP/dividend, keyed off the EXISTING state.dailyChallenge.streak via dailyChallengeDayInCycle() = ((streak-1)%7)+1 — no new cycle-tracking state, so a skipped day (streak resets to 1) restarts the cycle for free. Days 1-3: unchanged, dividend+VP only. Days 4-6: DAILY_DROP_CHANCE_MID (35%, PROVISIONAL) chance of one drop, split 50/50 (also provisional) between a common R&D material and a consumable — MSK's call: BOTH pools are daily-drop-eligible, not materials-only. Day 7: a GUARANTEED rare material — MSK's call, overriding the plan's own boosted-chance recommendation. New grantRdMaterial()/grantConsumable() are the FIRST writers to state.rdMaterials/state.consumables (fix313 shipped both schema-only); single mutation point each, same discipline as awardVP()/earnMoney(), and the ONLY call sites are inside the new rollDailyChallengeDrop(), so a drop decision and its state write can never come apart. Wired into completeRace()'s existing fix312/314 daily branch, rolled AFTER dc.streak's own update in that branch (dayInCycle must read the FRESH streak — a tick from 6→7 on this completion must roll as day 7, not day 6) and cached as dc.lastResult.dropAwarded (null on a no-roll day, absent/undefined on any pre-fix315 cached result — the recap card treats both the same). Calendar hub's completed-state card (dailyChallengeCardHtml) gains a third recap line, shown only when a drop landed. Math.random() per-roll, no seeding — same convention as the existing rollRaceBonus() (Race Retention Phase 1B), guarded the same way (fires at most once per completed day via the existing dc.lastDate!==todayKey gate) so re-rolling isn't a concern. NO NEW SAVE FIELDS beyond the dropAwarded key inside the already-unstructured dc.lastResult cache object (no schema/migration impact — lastResult has never had a fixed shape guard). No new or replaced assets, STATIC_ASSETS untouched. Diff confined to four sites: the new grant-funnels+drop-roll block (sited right after the CONSUMABLES registry), the roll call + lastResult field in completeRace()'s daily branch, and the recap card's new drop line — nothing else touched.
// const CACHE_VERSION = 'cim-v3.9.19';   // fix314: SPONSORSHIP DIVIDEND, HTML ONLY (CIM-plan-rewards-overhaul-v1 §1). The Daily Challenge's flat DAILY_CHALLENGE_CASH payout is replaced by a dynamic dividend: earnMoney() (the single money-mutation funnel) now also increments state.moneyEarnedToday; checkDailyReset() snapshots that running total into state.incomeLog (capped 7-entry, same cap discipline as ftpHistory/VP_LEDGER_CAP) at the existing day-boundary block, guarded on lastEnergyReset already being set so the very first-ever reset doesn't push a garbage 0-entry for the epoch. New computeSponsorshipDividend() — the single computation point for the formula — returns avg(incomeLog) * DIVIDEND_PCT (8%, PROVISIONAL, no real income-curve data exists yet), clamped between DAILY_CHALLENGE_CASH (now the floor, its role since fix312 changes here) and a 20x ceiling as a sanity rail against a single freak-income day skewing a <7-entry average. VP stays flat per MSK's confirmation of the plan's open question — DAILY_CHALLENGE_VP/awardVP() untouched. Wired into completeRace()'s existing daily-challenge branch: dividendAwarded replaces the flat constant in both the earnMoney() call and the cached dc.lastResult.cashAwarded. Recap text on the Calendar hub's completed-state card renamed "daily bonus" → "sponsorship dividend"; still reads lr.cashAwarded generically so the figure shown is always whatever actually landed. NO NEW SAVE FIELDS (moneyEarnedToday/incomeLog already existed, schema-only, since fix313) and no migration. No new or replaced assets, STATIC_ASSETS untouched. Diff confined to five sites: earnMoney(), the day-boundary block in checkDailyReset(), the new constants+function beside DAILY_CHALLENGE_CASH, completeRace()'s daily branch (call site + lastResult.cashAwarded), and the recap text string — nothing else touched.
// const CACHE_VERSION = 'cim-v3.9.17';   // fix312: DAILY CHALLENGE — REWARD FUNNEL + LASTRESULT RE-VIEW + STREAK/HISTORY + 7-PIP UI (Daily Challenges plan v1, "Reward funnel" + "Streak logic"). completeRace() gains a new `if(rd.isDailyChallenge){...}` branch, sited after the existing isCalEvent/gtStage branch (rd still valid, raceTypeNameForDisplay already computed) and before state.raceData is nulled — a genuinely ADDITIONAL branch alongside the normal !isCalEvent category-points/cooldown handling a daily already rides, not a replacement for it. Paid once per completed calendar day, keyed off state.dailyChallenge.lastDate: routes through the SAME single funnels as every other reward (awardVP(DAILY_CHALLENGE_VP,'daily_challenge') / earnMoney(DAILY_CHALLENGE_CASH), two new PROVISIONAL constants sited beside VP_WINDOW_CAP — retune on playtest, no real balance data exists yet). 'daily_challenge' is deliberately NOT in VP_UNCAPPED_PREFIXES, so it draws from the shared VP_WINDOW_CAP pool — the plan's entire grind-control mechanism, since entry itself is free. Streak reuses checkGrandTourLockouts()'s exact day-diff pattern: dayDiff===1 continues the streak, anything else (a skipped day, or no prior completion) resets it to 1. state.dailyChallenge.lastResult is cached (race type, course, position/fieldSize, race prize, VP/cash actually banked — awardVP's real capped-or-not return value, never the requested amount) and dailyChallengeCardHtml()'s completed state now re-views it instead of a generic "complete" line, falling back to the old generic text if lastResult is unexpectedly missing. history gets its usual capped-7 push/shift (cap discipline unchanged from fix310) as an audit trail, kept deliberately separate from the new 7-pip UI: new dailyStreakPipsHtml() reads state.dailyChallenge.streak directly (capped at 7 filled pips, "Nx" label beyond that) rather than walking .history, so a broken-then-restarted streak can never paint stale filled pips left over from a prior run — sited in the card in all three states (completed/locked/available), since streak persists independent of today's completion status. Resolves the plan's one remaining open UI question ("should a broken streak show acknowledgment beyond the pip row resetting?") as: silent, no separate toast — the row itself resetting is the acknowledgment. INFO_TEXTS.dailyChallenge extended with a reward/streak paragraph. No new state fields (fix310's load guard already covers lastResult/streak/history), no save migration, no new or replaced assets, STATIC_ASSETS untouched. Diff confined to: DAILY_CHALLENGE_VP/CASH consts beside VP_WINDOW_CAP, the new completeRace() branch, dailyStreakPipsHtml(), dailyChallengeCardHtml()'s completed-state bodyHtml + the new pip-row line in its return template, .daily-streak-* CSS beside .cal-start-btn, and the INFO_TEXTS.dailyChallenge body.
// const CACHE_VERSION = 'cim-v3.9.16';   // fix311: DAILY CHALLENGE — CALENDAR CARD + ENTRY GATE (Daily Challenges plan v1, "Entry flow"). New isDailyAttemptedToday() gate (reads state.dailyChallenge.lastDate only, deliberately independent of state.raceCooldowns) and startDailyChallenge(), which computes today's forced raceTypeId/courseId/shape from fix310's three seed functions and hands them to the REAL startRace() pipeline via a new opts param — NOT a parallel simulation path, per the plan's explicit risk-reduction call. startRace(raceTypeId, isClub, opts) gains opts.{isDailyChallenge, forceCourseId, forceShape}: when set, energy cost/deduction is skipped entirely (locked decision 4 — free entry), the per-race-type cooldown gate is bypassed (the daily's own gate is independent of it), the course is looked up by forced id against the same COURSES pool instead of the hourly rotation, and the race-shape roll is pinned to forceShape (the date-seeded pick) instead of the start-time-based one every other race uses — rollRaceShape() itself is unchanged. state.raceData now carries isDailyChallenge (default false, alongside the existing isClub) so completeRace()'s reward/streak branch (fix312) can identify a daily without a second copy of race-resolution logic. New dailyChallengeCardHtml(), a dedicated Calendar-hub card (same reasoning as grandTourCardHtml — no signup, no ev.prizes[] shape, its own gate) with three states: completed (isDailyAttemptedToday(), always false on a fresh save until fix312 writes lastDate — this render is forward-compatible, not fix312-dependent), locked (busy — a race is active elsewhere, reusing the existing raceActive||session.active gate), and available (previews today's forced race type + course, "Race Today's Challenge" button -> startDailyChallenge()). Wired into renderCalendarTab() directly below the season header per the plan's Nav placement decision — folded into the existing hub, not a new section. New INFO_TEXTS.dailyChallenge entry for the card's info button. No new state fields, no migration, no load guard change (fix310's dailyChallenge sub-keys are read-only here) — HTML ONLY, no new or replaced assets, STATIC_ASSETS untouched.
// const CACHE_VERSION = 'cim-v3.9.15';   // fix310: DAILY CHALLENGE — STATE + DETERMINISTIC SEED FUNCTIONS, NO UI YET (Daily Challenges plan v1). New state.dailyChallenge = {lastDate, lastResult, streak, history} — a daily-login-style hook deliberately separate from the energy economy, added with a per-sub-key `??` load guard (the tourProgress lesson) and serialized in saveGame after raceTelemetry. Three new pure, render-safe functions reuse getDailyWeather()'s exact seeded-RNG pattern (dayKey + hashStr + mulberry32/hash32) with a new salt per field, per the plan's determinism recipe: getDailyChallengeTypeId() picks which of the 4 RACE_TYPES today forces (hash32(hashStr(dayKey+'_dailytype'))%RACE_TYPES.length); getDailyChallengeCourseId(raceTypeId) reseeds getHourlyCourse()'s exact pool-index algorithm with a date+type salt instead of the rotation hour; getDailyChallengeShape(raceTypeId) calls the UNCHANGED rollRaceShape() with a date-based seed key instead of the start-time-based one startRace() uses. AUDIT FINDING written up per the plan's open item: simulateRaceResult() is confirmed NOT seeded (Math.random() in the player-score variance, field-size roll, non-rival AI FTP rolls, and AI score variance), but this does not gate fix311 — the function is called exactly once inside startRace() and its result is stored on state.raceData.result before the immediate save, so a mid-race reload never re-simulates; the daily's rival roll is exactly as reload-safe as any other race already is. No entry gate, no reward funnel, no Calendar UI — those are fix311/fix312. STATIC_ASSETS untouched, bump is for the state-shape change to ./index.html only.
// const CACHE_VERSION = 'cim-v3.9.14';   // fix309: R&D BUILD PREVIEW POLISH, HTML ONLY. The live stat preview in the R&D build card (renderRdBuildCard) now renders scaled stat-bar-outer/stat-bar-inner bars alongside the +Nw numbers, using the SAME 51/51 aero/climb scale renderBikeShop's owned cards use (previewMaxAero/previewMaxClimb, local consts — matches the existing per-section pattern, not a shared constant). Chosen over the numbers-only alternative after a mocked-up side-by-side comparison: the build card sits directly above the owned R&D Prototypes list, so the draft should read on the same visual scale as everything below it. Percentage math and markup mirror buildEquipCard's existing bar block exactly (Math.max(0,Math.min(100,...))), so a maxed single-stat prototype paints just shy of a full bar, consistent with the fix301/fix308 scale-calibration comments already in renderBikeShop. NO new state fields, no migration, no load guard, no new or replaced assets, STATIC_ASSETS untouched. Diff confined to renderRdBuildCard's body.
// const CACHE_VERSION = 'cim-v3.9.13';   // fix308: R&D BIKE BUILD. One custom-prototype build per season (state.rdBuildSeason gate), computed from 4 archetypes × 5 independent axes (972 combos) and stored as an immutable, append-only record per build in state.rdBikes — not catalogue entries, since the combo count rules out hand-authoring. resolveEquippedBike() is the new single funnel for equipped-bike lookups, replacing 5 call sites that used to do BIKES.find(...)||BIKES[0] directly, so a prototype resolves everywhere a catalogue bike does (race scoring, GT gear-tier check, idle-art tier, race setup card, workshop tuning target list). buildRdBike()/equipRdBike() are the sole mutation funnels for creation/equip. Two-tap arm-confirm on the build action (rdBuildConfirm, timestamp-based, mirrors buyVP's _shopBuyArmAt pattern) since it locks the season. HTML ONLY — no new or replaced assets, STATIC_ASSETS untouched.
// const CACHE_VERSION = 'cim-v3.9.11';   // fix306: LIFETIME GOALS — PER-TRACK INFO POPUP. Each of the 19 LIFETIME_GOALS entries gains a `desc` field (in-world voice, explains what the underlying statFn actually counts and how to move it) and the goal-track-head in renderGoalTracks() gains an ⓘ info-btn that calls the new openGoalInfo(trackId), which reuses the existing #info-overlay/closeInfo() markup (same overlay openInfo() already uses) rather than routing through INFO_TEXTS — the description lives on the LIFETIME_GOALS object itself, one source of truth, same reasoning as CAREER_GEAR_SOURCE being derived rather than hand-authored. openGoalInfo() appends one shared "this is a career total" footer to every track except perfectseason, which is explicitly season-scoped and says so in its own desc. HTML ONLY — no new state fields, no migration, no load guard, no new assets, STATIC_ASSETS untouched. Diff confined to: the 19 desc: lines inside LIFETIME_GOALS, the new openGoalInfo() function beside openInfo(), and the one new <button> line in renderGoalTracks().
// (was: const CACHE_VERSION = 'cim-v3.9.10';   // fix305: RIVAL DOSSIER EXPANSION, HTML ONLY. Four new derived rows on the existing rival-wrap card (renderRivalCard) — same card, same class names, just taller — no new hub section, no new sheet, per the fix301/fix302 "derive, don't store" rule. VO2 Max: reuses the exact aiVO2 = 30+(ftp-50)/500*55 formula the race-scoring loop already computes for AI opponents, applied to state.rival.ftp and clamped to MAX_VO2. Team: new RIVAL_TEAMS registry keyed by RACE_CATEGORIES id (same keying rule as RIVAL_TRAINING_BLOCKS, category-not-index so no array-position coupling), escalating from unattached/club to WorldTour-sounding names; picked by hashStr(rival name) only, so a promotion (which swaps the pool) is the only thing that moves it, not a re-render. Bike: new CATEGORY_TO_BIKE_TIER (explicit 7-entry array, category index -> BIKES tier, capped at tier 5 so the price:null fix301 career-unlock frames are never implied to be NPC-owned), rival's exact bike within that tier picked by hashStr(name). Last 3 races: the one row that ISN'T a fact about the rival (he has no real hidden race history — his finishing position is only ever computed on races the PLAYER runs, see rivalPosition in the scoring loop) — GENERATED per the fix305 comment block above getRivalVO2() in the HTML, using the same getRivalDossierDay()+hashStr(name)+category seed the training block already rotates on, salted per-slot by 2654435761, weighted to the front ~15% of a category-appropriate field size so a rival built to be genuinely strong (recalibrateRivalIfNeeded, 10-15% above player FTP at category entry) doesn't read as an average finisher. INFO_TEXTS.rival gains one paragraph saying plainly that the dossier is a snapshot of the rival\'s current level, not a scoreboard. NO NEW SAVE FIELDS, NO MIGRATION, NO LOAD GUARD — every value is derived at render time from data already in the save (rival name, rival ftp, raceCategory) plus, for the recent-races row only, the wall clock. Diff confined to three regions: the new registry/function block after getRivalTrainingBlock(), renderRivalCard()'s innerHTML, and the INFO_TEXTS.rival body. No new handlers, no new element IDs, no new or replaced assets, STATIC_ASSETS untouched.
// const CACHE_VERSION = 'cim-v3.9.09';   // fix304: GC FIELD REPAIR, HTML ONLY. fix296 fixed per-stage GC scoring (rivals now score AROUND the player's real finish rather than independently double-awarding the winner's points) but was deliberately not retroactive - a tour already in progress kept its old banked rival totals, which is "I won every stage but I'm not on the GC podium" for that one tour. New gcRepairFieldIfNeeded(tp, tourId), called as the first line of completeRace's gtStage branch (before this stage's own result is pushed), wipes the rival field's banked gcPoints and replays every PRIOR stage of that tour through the corrected gcScoreStageForField using the real recorded position from tp.stageResults. One-time via tp.gcFieldRepairedV2 - cannot distinguish old-bugged from already-correct from a bare save, so every in-progress tour is repaired once on its next stage; replaying an already-correct history reproduces an equally fair board, so this is safe unconditionally. Does NOT reach a tour that has already run its final stage and moved into state.calendar.completed - that result, bonus and any relic/jersey stay as paid. NO new save key beyond the one boolean flag (load-coerced beside the existing tp.gcField guard), no asset change, STATIC_ASSETS untouched.
// const CACHE_VERSION = 'cim-v3.9.08';   // fix303: TWELVE NEW COURSES, HTML ONLY. Three additional course variants APPENDED to each of the four COURSES pools (road / crit / hill / tt), taking the registry from 16 rows to 28. Appended to the end of each array, never inserted, so no stored courseId changes meaning and any in-progress race reloads onto the same course it started; getHourlyCourse() already selects by modulo over pool.length, so the rotation absorbs a 7-entry pool with no selector change. Each course carries the full existing shape - 24-point profile, commentary (7 lines with [TIME_DIFF] checks for tt, 5 for the rest) and three triggerPct 0.22/0.52/0.82 decision events of two choices each, every event exactly one defaultChoice:true. NO NEW SAVE FIELDS, no migration, no new handlers, no new element IDs. finishTerrainFor() classification RE-MEASURED with finishTerrainAudit() rather than estimated: 11 sprint / 6 drag / 11 summit across the 28 rows, no existing row's class changed and FINISH_TERRAIN_THRESHOLDS untouched; Viaduct Valley TT's tail was flattened after it first measured summit, because the summit class did not need a thirteenth member. The audit comment table is rewritten to the measured 28 rows. All course icons reuse existing sprite IDs (peak/rain/city/wind/brick/chart-up/mountain-bike/fire/cold/flag/road/crown) - NO new or replaced assets, STATIC_ASSETS untouched.
// const CACHE_VERSION = 'cim-v3.9.07';   // fix302: RIVAL DOSSIER, HTML ONLY. The Rival card gains two rows and two caption lines: the rival's career archetype (RIVAL_ARCHETYPES, fixed for the career because it is hashStr(rival name) % 4) and the training block he is currently working through (RIVAL_TRAINING_BLOCKS, four blocks per RACE_CATEGORIES id, re-rolled once per LOCAL calendar day). NO NEW SAVE FIELDS AND NO MIGRATION - both figures are DERIVED at render time from data already in the save (rival name, raceCategory) plus the wall clock, the same technique getHourlyCourse() uses for course rotation, so there is nothing to seed on old saves and no per-sub-key load guard to get wrong (the tourProgress lesson). RIVAL_TRAINING_BLOCKS is keyed by category ID, not index, so no index-map coupling to RACE_CATEGORIES; pools are read with a modulo and an uncat fallback, so a later fix can append blocks to any category without touching the selector. Category promotion swaps the pool wholesale, which is what makes the rival visibly level up alongside the player. RENDER-ONLY: getRivalArchetype() and getRivalTrainingBlock() read state and never write it, and renderRivalCard()'s existing recalibrateRivalIfNeeded() call is untouched. Diff confined to two regions - the new registry block after RIVAL_NAMES, and the renderRivalCard() innerHTML. No new handlers, no new element IDs, no new or replaced assets, STATIC_ASSETS untouched.
